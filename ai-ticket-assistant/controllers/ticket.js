  ...
  try {
    ...
    if (user.role !== "user") {
      ...
      tickets = await Ticket.find({ createdBy: user._id })
        .populate("assignedTo", ["email", "_id"])
        .select(
          "title description status priority relatedSkills assignedTo createdAt"
        )
        .sort({ createdAt: -1 });
    }
  ...
  try {
    ...
    if (user.role !== "user") {
      ...
      ticket = await Ticket.findOne({
        ...
        .populate("assignedTo", ["email", "_id"])
        .select(
          "title description status priority helpfulNotes relatedSkills assignedTo createdAt"
        );
    }
...
export const resolveTicket = async (req, res) => {
  try {
    ...
    // Only the creator, a moderator, or an admin can resolve
